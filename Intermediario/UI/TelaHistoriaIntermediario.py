# Intermediario/UI/TelaHistoriaIntermediario.py
import pygame
import random
import string
import math

class TelaHistoriaIntermediario:
    """
    Tela de transição para o Intermediário:
      - Fundo 'code rain' + painel translúcido para legibilidade
      - Texto com efeito typewriter, dots, setas, botão pulsante
      - Página 2 com 3 cartões clicáveis (com wrap no texto)
      - Enter/Espaço/→ para avançar, ←/Backspace para voltar, S para pular
    """
    def __init__(self, largura, altura, on_confirmar):
        ...
        # cores
        self.col_txt      = (242, 246, 255)     # mais forte para não ofuscar
        self.col_txt_dim  = (210, 218, 230)
        self.col_panel    = (16, 22, 34, 220)   # painel do texto mais opaco
        self.col_panel_t  = (18, 26, 40, 170)   # painel do título
        self.col_panel_bd = (90, 150, 255, 130)
        self.col_dot_on   = (120, 190, 255)
        self.col_dot_off  = (80, 105, 140)

        # fundo + code rain
        self.bg_surface = pygame.Surface((largura, altura))
        self._fill_vertical_gradient(self.bg_surface, (10, 10, 20), (16, 24, 40))
        ...
        self.paginas = [
            {
                "titulo": "Parabéns! Novo Patamar",
                "texto": (
                    "Você concluiu a etapa Iniciante. Agora começa o Intermediário: "
                    "desafios mais profundos, lógica mais afiada e novas formas de evoluir."
                )
            },
            {
                "titulo": "Novas Oportunidades",
                "texto": (
                    "Você desbloqueia o modo Freelancer, cursos/loja e um HUD de progresso. "
                    "Clique nos cartões abaixo para saber mais."
                )
            },
            {
                "titulo": "Pronto para Avançar",
                "texto": (
                    "A cada tópico: 4 desafios + 1 minigame. Evolua backend/frontend, "
                    "ganhe recompensas e invista no seu crescimento."
                )
            },
        ]
        self.idx_pagina = 0

        # geometria do conteúdo principal
        self.max_w = int(self.largura * 0.70)
        self.content_x = self.largura // 2 - self.max_w // 2
        self.content_y = 180
        self.content_h = 260

        # cartões interativos (página 2) — com dimensões ajustáveis e wrap
        self.card_w = max(260, min(340, int(self.largura * 0.24)))
        self.card_h = 104
        self.cards = {
            "freelancer": {
                "rect": pygame.Rect(0, 0, self.card_w, self.card_h),
                "titulo": "Freelancer",
                "desc": "Pegue projetos, entregue código e ganhe recompensas."
            },
            "cursos": {
                "rect": pygame.Rect(0, 0, self.card_w, self.card_h),
                "titulo": "Cursos & Loja",
                "desc": "Compre cursos e itens; progrida enquanto joga."
            },
            "hud": {
                "rect": pygame.Rect(0, 0, self.card_w, self.card_h),
                "titulo": "HUD & Skills",
                "desc": "Acompanhe backend/frontend e outras métricas em tempo real."
            },
        }
        self._hover_card = None

        # typewriter
        self._type_speed_cps = 52.0
    ...
    def _blit_text_with_shadow(self, tela, fonte, texto, cor, pos, shadow_offset=(2, 2)):
        ...
        tela.blit(sombra, (pos[0] + shadow_offset[0], pos[1] + shadow_offset[1]))
    ...
    def _update_code_rain(self, dt):
        ...
        for d in self.drops:
            ...
            self.rain_surface.blit(self._glyph_cache[d["char"]], (d["x"], d["y"] - self.gh))
            self.rain_surface.blit(self._head_cache[d["char"]],  (d["x"], d["y"]))

    # ---------- texto ----------
    def _rebuild_wrap_cache(self):
        self._wrap_cache.clear()
        for i, pag in enumerate(self.paginas):
            self._wrap_cache[i] = self._wrap(pag["texto"], self.fonte_texto, self.max_w)
    ...
    def _wrap_lines(self, fonte, texto, largura_max):
        """Quebra texto em múltiplas linhas dentro de largura_max."""
        return self._wrap(texto, fonte, largura_max)

    def _render_wrapped_lines(self, tela, fonte, linhas, cor, x, y, line_gap=4, max_lines=None):
        count = 0
        last_w = 0
        for i, linha in enumerate(linhas):
            ...
            tela.blit(surf, (x, y))
            last_w = surf.get_width()
            y += fonte.get_height() + line_gap
            count += 1
        return count, last_w
    ...
    def _draw_page_text_typewriter(self, tela, idx, dt):
        ...
        # cartões na página 2 (somente quando a página está sendo mostrada)
        if self.idx_pagina == 1:
            self._draw_cards(tela)

    # ---------- cartões p/ pág. 2 ----------
    def _draw_cards(self, tela):
        gap = 18
        w = self.card_w
        h = self.card_h
        total_w = w * 3 + gap * 2
        start_x = self.largura // 2 - total_w // 2
        y = self.content_y + self.content_h + 22

        order = ["freelancer", "cursos", "hud"]
        mx, my = pygame.mouse.get_pos()
        self._hover_card = None

        for i, key in enumerate(order):
            rect = self.cards[key]["rect"]
            rect.update(start_x + i * (w + gap), y, w, h)

            hover = rect.collidepoint(mx, my)
            fill = (30, 42, 60, 230) if not hover else (50, 90, 145, 238)
            bord = (110, 170, 255, 150) if hover else (110, 170, 255, 110)
            self._draw_panel(tela, rect, fill, bord, radius=12, border_w=2)

            # título
            title = self.cards[key]["titulo"]
            self._blit_text_with_shadow(tela, self.fonte_ui, title, (255, 255, 255),
                                        (rect.x + 12, rect.y + 10))

            # descrição com wrap (2 a 3 linhas, conforme couber)
            pad = 12
            desc = self.cards[key]["desc"]
            max_w = rect.w - pad * 2
            max_lines = max(2, (rect.h - (10 + self.fonte_ui.get_height() + 6) - pad) // (self.fonte_ui.get_height() + 2))
            linhas = self._wrap_lines(self.fonte_ui, desc, max_w)

            # se exceder, corta e coloca "…" na última
            if len(linhas) > max_lines:
                linhas = linhas[:max_lines]
                # adiciona "…" no fim da última linha
                if linhas:
                    ult = linhas[-1]
                    # tenta colocar "…" cabendo
                    while self.fonte_ui.size(ult + "…")[0] > max_w and len(ult) > 0:
                        ult = ult[:-1]
                    linhas[-1] = ult + "…"

            text_y = rect.y + 10 + self.fonte_ui.get_height() + 6
            for ln in linhas:
                surf = self.fonte_ui.render(ln, True, (225, 235, 245))
                tela.blit(surf, (rect.x + pad, text_y))
                text_y += self.fonte_ui.get_height() + 2

            if hover:
                self._hover_card = key
            ...
            if e.type == pygame.MOUSEBUTTONUP and e.button == 1:
                if self.botao_rect.collidepoint(e.pos):
                    if self._is_page_fully_shown():
                        ...
                        self._shown_chars = self._page_text_len(self.idx_pagina)
                    ...
                    # dots
                    for i, r in enumerate(getattr(self, "_dot_rects", [])):
                        if r.collidepoint(e.pos):
                            if i != self.idx_pagina:
                                ...
                                self._fade_alpha = 255
                            break
                    # clique no painel de texto avança/termina typewriter
                    text_panel = pygame.Rect(self.content_x - 16, self.content_y - 14,
                                             self.max_w + 32, self.content_h + 28)
    ...
    def desenhar(self, tela):
        ...
        # botão (pulsante com clamp de cor)
        ...
        btn_color = self._mul_color(base, pulse)  # evita extrapolar >255
        pygame.draw.rect(tela, btn_color, self.botao_rect, border_radius=14)
        label = "Vamos lá! (Enter)" if (self.idx_pagina == len(self.paginas) - 1 and self._is_page_fully_shown()) else "Continuar (Enter)"
        self._blit_text_with_shadow(
            tela, self.fonte_texto, label, (255, 255, 255),
    ...
    def _draw_arrows(self, tela):
        ...
        self._draw_panel(tela, self.arrow_prev,
                         (20, 28, 42, 160 if prev_hover else 110),
                         (110, 170, 255, 150 if prev_hover else 110),
                         radius=12, border_w=2)
        self._draw_arrow_icon(tela, self.arrow_prev, left=True)
        # next
        next_hover = self.arrow_next.collidepoint((mx, my))
        self._draw_panel(tela, self.arrow_next,
                         (20, 28, 42, 160 if next_hover else 110),
                         (110, 170, 255, 150 if next_hover else 110),
                         radius=12, border_w=2)
